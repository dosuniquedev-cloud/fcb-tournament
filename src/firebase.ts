...
export const signInWithGoogle = async () => {
  if (isFirebaseConfigured && auth) {
    try {
      ...
      console.warn("Firebase Auth fallback triggered:", error?.code || error);
      // Seamless fallback so player registration is NEVER blocked by auth configuration
      const mockUser = {
        uid: 'player_' + Date.now(),
        displayName: 'FCB Player',
